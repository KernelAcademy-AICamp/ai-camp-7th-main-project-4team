...
PARTS = [
    ...
    # 배 — 브랜드가 사이즈표에 안 적는 미표기 부위인데 회귀 r²이 가장 높다(0.85). 진단은 이미 '배'를
    #   fit으로 묻고 엔진에 TOP:belly 밴드도 있는데 몸쪽 회귀가 없어 죽어 있던 신호. 로우라이즈·턱인 핏의 몸쪽 기준.
    ("belly",      "배둘레",          "배둘레",           "bellyCircumference"),
    # 젖가슴아래둘레(underbust) — 여성 전용(남성 컬럼 0 → build가 자동 스킵). 브라 밴드둘레의 몸쪽 기준.
    #   여성이 유일하게 숫자로 아는 치수(브라 사이즈)로 재인 입력받아 이 추정을 덮어씀. 컵볼륨=젖가슴−젖가슴아래.
    ("underbust",  "젖가슴아래둘레(여)", "젖가슴아래둘레",  "underbustCircumference"),
    # 미표기 병목(flag) 부위의 몸쪽 기준 — 브랜드 사이즈표에 없고, 진단이 '꼈다/괜찮다'로만 수집하던
    #   페인 플래그(팔소매통·목·암홀·종아리)를 '측정 축'으로 승격하는 토대. A축 추정(docs/6 §4)의 재료.
    #   소매통=위팔둘레(upperArm, 위에 이미 있음)에 대응. 아래 셋을 추가.
    ("neck",       "목둘레",          "목둘레",           "neckCircumference"),
    ("armhole",    "겨드랑둘레",      "겨드랑둘레",       "armholeCircumference"),
    ("calf",       "장딴지둘레",      "장딴지둘레",       "calfCircumference"),
