  ...
  function reqAction(r,i){ var s=r.status;
    ...
    if(s==='결제대기') return '<div class="reqact"><div class="offerbox"><b>결제하고 시작하기</b><div class="omsg">'+(r.nm||'')+' 쇼퍼로 매칭됐어요 · 에스크로 결제 후 코디를 시작해요</div></div><button class="tinybtn" style="margin-left:auto" onclick="openPay('+i+')">결제하기 →</button></div>';
  ...
  function reqAccept(i){ reqs[i].status='결제대기'; saveLS('reqs',reqs); renderReqs(); openPay(i); toast((reqs[i].nm||'')+' 쇼퍼가 수락했어요 · 결제 후 시작해요'); }
  ...
  function reqActions(r,i){ var s=r.status;
    ...
    if(s==='환불') return '<div class="rq-btns"><button class="tinybtn ghost" onclick="closeBids();go(\'shop\')">다른 쇼퍼 찾기</button></div>';
  ...
  function progressSectionsHTML(r, i){
    ...
    var deliver='<div class="rq-sec"><div class="rq-h">받은 결과물</div><div class="dgal">'+gal+'</div>'+
      ...
      '<div class="rs-lines">'+DELIVER_LINKS.map(function(l){ return '<div class="rs-row"><span><b style="color:var(--ink)">'+l.brand+'</b> '+l.name+' · '+l.size+' · <span class="num">'+l.price.toLocaleString()+'</span>원</span><a onclick="toast(\'데모 · 실제 구매 링크는 쇼퍼가 코디와 함께 작성해요\')" style="color:var(--green);font-weight:700;cursor:pointer;white-space:nowrap">보러가기 ›</a></div>'; }).join('')+'</div>'+
      (r.status==='진행중'
        ? '<button class="btn" style="width:100%;margin-top:14px" onclick="reqComplete('+i+')">받았어요 · 완료 확인</button>'+
          '<p style="text-align:center;margin:9px 0 0;color:var(--sub2);font-size:12px">완료 확인 시 에스크로 결제금이 쇼퍼에게 정산돼요</p>'+
  ...
  function settlementSectionHTML(r){
    ...
    return '<div class="rq-sec"><div class="rq-h">정산 · 영수증</div>'+
      '<div class="statban go"><span class="sb-ic">'+stIcon('lock')+'</span><div class="sb-tx"><b>에스크로 정산 릴리스</b><p>보관된 결제금 '+price.toLocaleString()+'원이 '+(r.nm||'쇼퍼')+' 쇼퍼에게 지급돼요</p></div></div>'+
  ...
  function renderDispute(){
    ...
    var warn='<div class="rq-sec"><div class="statban no"><span class="sb-ic">'+stIcon('lock')+'</span><div class="sb-tx"><b>접수 시 정산 릴리스 보류</b><p>결제금이 묶이고, 쇼퍼 소명 후 관리자가 중재해요</p></div></div></div>';
  ...
  function disputeSectionHTML(r){ var d=r.dispute||{};
    return '<div class="rq-sec"><div class="rq-h">분쟁 · 중재</div>'+
      ...
      '<div class="note" style="margin-top:8px">관리자 분쟁 큐(3.B.4)와 연계 · 쇼퍼 소명 후 중재</div></div>';
  }
  ...
  function confirmAward(idx){ var e=EX[idx];
    askConfirm('<b>'+e.nm+' 쇼퍼</b>로 선택할까요?<div class="cf-sub">선택 후 에스크로 결제를 하면 코디를 시작해요</div>', '이 쇼퍼로 선택', function(){ awardBid(idx); }); }
  function awardBid(idx){ var r=reqs[_bidReq]; if(!r) return; var e=EX[idx];
    ...
    saveLS('reqs',reqs); renderReqs(); openPay(_bidReq); toast(e.nm+' 쇼퍼 선택 완료 · 결제 후 시작해요');
  }
  ...
  function renderPay(){
    ...
    var shopperHead = e ? '<div class="rs-head"><span class="rs-ic"><img src="'+img(e)+'" style="width:34px;height:34px;border-radius:50%;object-fit:cover" onerror="'+FB+'"></span><div class="rs-htx"><b class="rs-title">'+e.nm+' 쇼퍼</b><span class="rs-sum">'+svcLabel(r.svc)+(r.occ&&r.occ.length?' · '+r.occ.join('·'):'')+'</span></div></div>' : '';
    ...
    var escrow='<div class="rq-sec"><div class="statban wait"><span class="sb-ic">'+stIcon('lock')+'</span><div class="sb-tx"><b>에스크로 안전결제</b><p>결제금은 안전 보관되고, 완료 확인 후 쇼퍼에게 정산돼요</p></div></div></div>';
  ...
  function payConfirm(){ var r=reqs[_bidReq]; if(!r) return;
    ...
    saveLS('reqs',reqs); renderReqs(); openReqDetail(_bidReq); toast('결제 완료 · '+(r.nm||'')+' 쇼퍼와 코디를 시작해요');
  }
  /* 공용 확인 모달 */
