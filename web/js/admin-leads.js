/* admin-leads.js — 전문가 수요(lead) 집계. [db/02]
   페이크도어 = 스타일리스트 웨이트리스트 오픈알림(이메일). api에선 kind=notify·이메일만 채워지고
   occasion·budget·note·stylist·견적(quote)은 목업 플로우라 미도달 → 화면은 실측 스키마(진단전환·이메일·추세)에 맞춤.
   RLS admin 읽기. */
(function(){
  "use strict";
  ...
  function render(leads, diagSet, diagCount){
    ...
    // 유입 게이트 · 서비스 (occasion은 페이크도어에서 미수집이라 제외 — 게이트·서비스로 분해)
    ...
    $('segTable').innerHTML='<thead><tr><th>구분</th><th>신청</th></tr></thead><tbody>'+
      ...
      '<tr><td class="muted" colspan="2" style="padding-top:12px;border-top:1px solid var(--line)">서비스별</td></tr>'+
    ...
    // 수요 목록 — 유형(항상 notify)·상황(항상 null) 컬럼 제거. 시각·서비스·이메일·유입만.
    ...
    $('ldTable').innerHTML='<thead><tr><th>시각</th><th>서비스</th><th>이메일</th><th>유입</th></tr></thead><tbody>'+(rows||'<tr><td class="muted" colspan="4">아직 수요가 없어요</td></tr>')+'</tbody>';
  }
