#!/usr/bin/env python3
"""문서.html 재생성기 — md 5종을 뷰어 템플릿(_tpl.html)에 주입한다.

원칙: 내용은 .md가 원본, _tpl.html은 렌더 껍데기(스타일·렌더러). 둘 다 git에 보관.
사용법: python3 docs/문서생성.py   (docs/ 안에서 실행)
"""
import html, pathlib
...
for did, fn, label in DOCS:
    txt = (d / fn).read_text(encoding="utf-8").replace("</script>", "<\\/script>")
    blocks.append(
        f'<script type="text/markdown" data-doc="{did}" '
        f'data-file="{html.escape(fn)}" data-label="{html.escape(label)}">\n{txt}\n</script>'
    )

tpl = (d / "_tpl.html").read_text(encoding="utf-8")
out = tpl.replace("__DOCS__", "\n".join(blocks))
(d / "문서.html").write_text(out, encoding="utf-8")
print(f"wrote 문서.html ({len(out)} bytes) from _tpl.html + {len(DOCS)} md docs")
